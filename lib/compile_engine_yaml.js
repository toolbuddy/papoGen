...
compile_engine_yaml.gen_doc_yaml = function(src_path, title, out_path, theme) {
    // fetch and generate
    const files = fh.create().paths(src_path).ext('yaml').find((err, files) => {
        if (err)
            ...
            console.dir(files);
            let fnamelist = utils.fetch(files, "yaml");
            ...
            let result = template_api.template("doc")({ title: title, namelist: fnamelist, moment: moment, theme: theme });
            // success
...
compile_engine_yaml.gen_resume_yaml = function(src_path, title, out_path, theme) {
    // fetch and generate
    const files = fh.create().paths(src_path).ext('yaml').find((err, files) => {
        if (err)
            return;
            ...
            let result = template_api.template("resume")({ title: title, namelist: fnamelist, moment: moment, theme: theme });
            // success
            console.log(`Successfully compile/render target pug files. Ready to output.`);
            // Write file 
            mkdirp(out_path, function(err) {
