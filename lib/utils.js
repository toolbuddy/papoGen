...
const md = require('markdown-it')({
    replaceLink: function(link,env){
        // empty link (e.g. image link, hyper link)
        if(link.length == 0){
            console.log("[Debug] Detect empty link.")
            return link;
        }
...
// output & copy asset
utils.render = function(out_path,result,theme){
    mkdirp(out_path, function(err) {
        if (err) {
            ...
            // copy theme
            if(theme=='paper'){
                fsx.copySync(path.join(__dirname, 'theme',theme,'monokai.css'),path.join(out_path,'asset','monokai.css'))
                fsx.copySync(path.join(__dirname, 'theme',theme,'paper.min.css'),path.join(out_path,'asset','paper.min.css'))
            }
            else if(theme=='papogen'){
                fsx.copySync(path.join(__dirname, 'theme',theme,'papogen.min.css'),path.join(out_path,'asset','papogen.min.css'))
            }
            else{
                console.log(`[papoGen][Render Process] Specified theme: '${theme}' not supported yet! Exit the process.`)
                exit(1);
            }
        }
    })
