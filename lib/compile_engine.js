...
compile_engine.gen_doc = function(src_path,title,out_path,gen_base,theme){
    ...
    switch(gen_base){
        case 'json':
            gen_doc_json(src_path,title,out_path,theme);
            break;
        case 'yaml':
...
compile_engine.gen_resume = function(src_path,title,out_path,gen_base,theme){
    ...
    switch(gen_base){
        case 'json':
            gen_resume_json(src_path,title,out_path,theme);
            break;
        case 'yaml':
...
function gen_doc_json(src_path,title,out_path,theme){
    // fetch and generate
...
function gen_resume_json(src_path,title,out_path,theme){
    // fetch and generate
    const files = fh.create().paths(src_path).ext('json').find((err,files) => {
        if(err)
            return;
